...
- [`cmd/hamster`](cmd/hamster/) — the server binary. `hamster serve` is a single-node dev preview, but a durable one: blobs under `<data-dir>/blobs`, metadata in BadgerDB under `<data-dir>/meta` — restarts keep everything.
