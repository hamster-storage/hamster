...
- [`internal/raftnode`](internal/raftnode/) — etcd-io/raft's `RawNode` driven over the seam ([ADR-0012](docs/adr/0012-etcd-raft-consensus-library.md)): WAL-backed storage, transport glue, apply into `meta.Store`, and the election timer Hamster owns for determinism ([ADR-0024](docs/adr/0024-deterministic-election-timing.md)). Proven by three-node cluster schedules under the simulation harness; not yet wired into `hamster serve` (single node until v0.2 ships).
