...
- [`internal/gateway`](internal/gateway/) — the S3 HTTP front end ([S3-API.md](docs/S3-API.md)): SigV4 wiring, path-style routing, XML envelopes, and handlers that turn requests into metadata proposals and blob I/O. The store and rng are loop-owned; handlers post to the loop and wait. Tested end to end with an independently written client-side signer.
