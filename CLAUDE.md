...
- [`internal/blob`](internal/blob/) — the v0.1 single-node data path: whole-file blobs on a `seam.Disk`, addressed by `DataID`, durable (written and synced) before the metadata commit. Replaced by the erasure-coded path behind the gateway's `BlobStore` interface.
