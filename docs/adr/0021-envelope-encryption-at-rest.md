...
6. **Every part of this is in the open-source binary, permanently.** If Hamster ever grows a commercial edition, the line is drawn here, in advance: **data security is never gated.** Encryption, key management, TLS, object lock, and integrity verification stay in the open core — the project's origin story is what happens to users when capabilities they depend on end up on the wrong side of a fence, and how safe the stored bytes are is never that capability. Commercial offerings, if any, live in organizational integration and services (for example single sign-on for the console, SIEM export, fleet management, support), and the line only ever moves in the open direction: a feature shipped in the open core is never withdrawn behind a paid one.
