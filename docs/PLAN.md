...
- **Transition tracking + manual rebalance** — the prerequisite that makes any
  ...
  1. the leader's reconcile *opens* a transition (sets `Previous = prior member
     ...
     instead of swapping the layout outright. Couple this with the close (below):
     never open a transition that cannot be closed — the model holds one (old,new)
     pair, not a stack, so a second change must wait for the first to finish.
  2. repair migrates shards old→new during a transition and *closes* it (drops
     `Previous`) once a sweep finds nothing left to move. **Migration is a copy,
     not an EC reconstruct**: during a transition a shard sits at its *old* home,
     not its new one, so there is nothing at the new positions to reconstruct
     *from* — repair fetches the whole shard from `old[i]` and writes it to
     `new[i]` (then reclaims the old copy, or leaves it an orphan for GC). EC
     reconstruction stays the path only for a shard *damaged* mid-move (clean at
     neither home), rebuilt from any k available across old+new. This is the
     meatiest, most durability-critical slice and must clear the simulation crash
     schedules (invariant 5), including crash-mid-migration convergence.
