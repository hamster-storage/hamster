...
Control plane and data plane both run over mutual TLS, always — a cluster CA is minted at `init`, nodes get certificates at join, and there is no plaintext cluster mode ([ADR-0022](adr/0022-cluster-mtls.md)). The TLS layer lives in the transport adapter, below the simulator seam.
...
**Discovery is not a subsystem — membership is metadata.** The Raft configuration plus the cluster layout ([ADR-0004](adr/0004-partitioned-placement.md)) are the authoritative record of who is in the cluster, replicated like all metadata. A new node joins by contacting any existing node (`hamster join <addr>`), which proposes a membership change through Raft; from then on every node learns its peers from the replicated layout. Health checking runs among known members and feeds repair and the upgrade interlock. No gossip protocol, no external registry.
