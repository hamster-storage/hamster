...
## Encryption at rest (SSE-S3) — what is supported, and what is not

Hamster offers **SSE-S3**: server-managed encryption at rest ([ADR-0021](adr/0021-envelope-encryption-at-rest.md)), surfaced on the standard `x-amz-server-side-encryption: AES256` header.

- **Keys are cluster-managed, not per-request.** Whether an object is encrypted is decided by the cluster's replicated encryption posture (an operator turns it on with `cluster encrypt`), not by the request header. When the cluster encrypts, every new object is encrypted with a fresh per-object key, and `x-amz-server-side-encryption: AES256` is reported back on PUT, GET, and HEAD. Existing objects written before encryption was enabled stay readable and unencrypted — a cluster holds a mix, and each object's record says which it is.
- **An honest refusal beats a silent lie.** A request that asks for `AES256` when the server is not encrypting (the single-node preview, or a cluster with the posture off) is **refused** (`501 NotImplemented`) rather than storing plaintext under the impression it is protected. SSE-KMS (`aws:kms`) and SSE-C (customer-provided keys, the `x-amz-server-side-encryption-customer-*` headers) are **not supported** and refused honestly; the per-object-key machinery leaves room for SSE-C later.
- **Encryption is enable-only.** Once a cluster starts encrypting it cannot be switched back to plaintext in place — there is no disable. To stop encrypting, migrate the data to a fresh cluster.
- **Single-node `serve` does not encrypt.** Encryption at rest is a cluster feature in v0.7; the single-node dev preview refuses SSE requests rather than pretend.
- **Not a key-management or compliance certification.** Hamster encrypts object data at rest and never persists or transmits the master key (KEK), but it has not been assessed or certified for any regulation, and key custody/rotation specifics are the operator's responsibility (mount the key off the data disk).
